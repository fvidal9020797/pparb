<?php
	...
	if ((isset($_SESSION['MM_Username']) && trim($_SESSION['MM_Username'] != "")))  {
		$cn = pg_connect("host={$hostname_mdryt} port={$port} dbname={$database_mdryt} user={$_SESSION['MM_Username']} password={$_SESSION['MM_Password']}") or trigger_error("Nombre de Usuario o Contraseña  no validos.",FATAL);
	}
	elseif(isset($username_mdryt)) {
		$cn = pg_connect("host={$hostname_mdryt} port={$port} dbname={$database_mdryt} user={$username_mdryt} password={$password_mdryt}") or trigger_error("Nombre de Usuario o Contraseña  no validos.",FATAL);
	}else{
	  include("logout.php");
	}
