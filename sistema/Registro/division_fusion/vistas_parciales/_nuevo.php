...
function agregar_fila(data){
	if(data!="0::::"){
		if(validacion_post(data.split(":")[4])==false){
		    ...
		    var cell4 = row.insertCell(3);//DELETE.GIF
		    var element4 = document.createElement("input");
		    element4.setAttribute("src","../../images/DELETE.GIF");
		    element4.setAttribute("id","del"+rowCount);
		    element4.setAttribute("onclick","borrar_fila('"+rowCount+"')");
		    element4.type = "image";
...
function validacion_post(estado){
  ...
  //alert(estado);
	if(estado=="347"){
		$("#predio_no_encontrado").html("ERROR: No se puede realizar la Fusion ya que el predio se encuentra CANCELADO");
		$("#predio_no_encontrado").css({
			"display":"block"
		});
		result = true;
	}else if(estado=="240"){
		$("#predio_no_encontrado").html("ERROR: El Predio se Encuentra con Suspencion Temporal");
		$("#predio_no_encontrado").css({
			"display":"block"
		});
		result = true;
	}else if(estado=="360"){
		$("#predio_no_encontrado").html("ERROR: No se Puede fusionar ya que el Predio ya esta Fusionado");
		$("#predio_no_encontrado").css({
			"display":"block"
		});
		result = true;
	}else if(estado=="167"){
		$("#predio_no_encontrado").html("ERROR: No se puede Fusionar un Predio Con Preliquidacion.");
		$("#predio_no_encontrado").css({
			"display":"block"
		});
		result = true;
		...
		$("#predio_no_encontrado").html("ERROR: No se puede fusionar un predio en Evaluacion");
		$("#predio_no_encontrado").css({
			"display":"block"
		});
		result = true;
    ...
    $("#predio_no_encontrado").html("ERROR: No se Puede fusionar ya que el Predio esta Dividido.");
    $("#predio_no_encontrado").css({
      "display":"block"
    });
    result = true;
...
<div style="text-align: -webkit-center;  height: auto;">
	...
	<table border="0" class="table">
		...
		<tr>
			<td valign="bottom">
				<h5>Lista de predios a fusionar<span class="label label-default"></span></h5>
			</td>
		</tr>
		<tr>
			<td colspan="2">
