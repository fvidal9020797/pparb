<?php session_start();
  ...
  include "../reportes/Formulario_Contrato.php";
if(isset($_REQUEST['anular']))
{
  $iPago= intval($_REQUEST['idpago']);
   $sql_anulacion="UPDATE pagosrealizados
   SET  fechaanulacion=CURRENT_DATE
 WHERE idpago=".$iPago." and idregistro=".$_SESSION['idreg'].";";
  $anulacion = pg_query($cn,$sql_anulacion);
}
...
if(isset($_REQUEST['imprimircontrato']))
{
	$CodigoParcelas=$row_parcela['idparcela'];
	$CodigoDeRegistro= intval($_SESSION['idreg']);

	$sql_actualizar="select f_actualizar_plan_pago(".$_SESSION['idreg'].")";
	$update = pg_query($cn,$sql_actualizar);


	//echo ($CodigoParcelas);
	//echo ($CodigoDeRegistro);
	ImprimirContrato($CodigoParcelas,$CodigoDeRegistro);
}
...
<script language="JavaScript" src="../libraries/jquery-ui-1.10.2/jquery-1.9.1.js"></script>
...
<?php
        ...
        <div align="center" class="texto">
          ...
          <p><br>
          <b><big>CONSTANCIA DE PAGO / RECIBO DE INGRESO</big></b>
          <br>
          </p>
              ...
              <table width="100%" border="0" class='taulaA' cellspacing="0" cellpadding="0">
                ...
                <tr>
                  <td height="22" colspan="5" class="noborder" id="blau9"><table width="97%" border="1" align="center" cellspacing="0" cellpadding="0" class="taulaA" id="dataTable">
                    <tr align="center">
                      ...
                      <td width="22%" class="cabecera2">Fecha de Pago</td>
                      <td width="7%" class="cabecera2">Número de Cuota </td>
                      <td width="14%" class="cabecera2">Número de Boleta Pago</td>
                      <td width="10%" class="cabecera2">Monto Cancelado en Bs</td>
                      <td width="18%" class="cabecera2">Pago Aproximado en UFV </td>
                      <td width="18%" class="cabecera2">Número de Recibo</td>
                      <td width="8%" class="cabecera2">Imprimir Boleta</td>
                    </tr>
				 ...
				 ?>
                 <?php if(isset($row_cuotas['fechaanulacion'])){
                ...
                <?php  if(isset($_SESSION['datos_recibo']['FechaPago'.$i]) && ($_SESSION['datos_recibo']['FechaPago'.$i] == $row_ufv['idvalorufv'])){
                 ...
                 > <?php echo $row_ufv['fecha_ufv'].' valor ufv :'.$row_ufv['valor_ufv']?></option>
					  ...
					  <td>
				          <a href="N_Recibo.php?imprimir=1&idpago=<?php echo $row_cuotas['idpago'] ;?>"><img width="18" src='../images/logosdos/impresora2.png'>  </img> </a>
               <?php if(isset($row_cuotas['fechaanulacion'])){
                                         echo "ANULADO";
                                        }else{
                                        if((strrpos($_SESSION['MM_Rol'],'3')== true)){  ?>
             <a class="anular" href="javascript:void(0)" h="N_Recibo.php?anular=1&idpago=<?php echo $row_cuotas['idpago'] ;?>"><img align="right" width="18" src='../images/delete.jpg'>  </img> </a>
                                         <?php
			...
			?>
              ...
              <table width="100%" border="0">
                <tr>
				  ...
				  <?php
				   if($row_modalidadPago['modalidad'] != 'Pago Unico')
				     {
					?>
				  <td
					class="taulaA" width="10%"> <a type="button" class='cabecera2' href="N_Recibo.php?imprimircontrato=1"> <Strong> Imprimir Contrato </strong> </img> </a>
				  </td>
...
$(document).ready(function(){
  ...
  $(".anular").click(function() {
    ...
    var a=$(this).parent().parent();
     var n= a.find('td').eq(6).html();
      var answer = confirm("¿Esta Seguro de que desea Anular Recibo con numero="+n+"?");
  if (answer){
    window.location = ref;
  }
  else{
...
<div align="center" class="texto"><p>&nbsp;</p></td>
                </tr>
              </table>
