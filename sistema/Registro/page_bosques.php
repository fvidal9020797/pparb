...
$sql_suprestfuera="SELECT * FROM superficieregistro WHERE idtiposuperficie=343 AND idregistro=".$_SESSION['idreg'];
$supfuera = pg_query($cn,$sql_suprestfuera);
$row_supfuera= pg_fetch_array ($supfuera);
...
if ((isset($_POST["New_Predio"])) && ($_POST["New_Predio"] == "Guardar")){
	   ...
	   /*********************************Validando Especies a Restituir****************************************/
		...
		/* elseif( ($_SESSION['datos_bosque']['Superficie']>50) && ((round($_SESSION['datos_bosque']['suptptotalreftpfp'],4)) < (round($_SESSION['datos_bosque']['suptpfp']*0.1,4)) )      )
 		 {
			 $mensajeerror="VERIFIQUE: La superficie total para la restitucion de Bosques en TPFP es menor a la minima requerida (10%) equivalente a = ".$_SESSION['datos_bosque']['suptpfp']*0.1." Ha";
		 } */
		...
		elseif ($_SESSION['Causal']==2){
		    ...
		    elseif($_SESSION ['datos_bosque']['suptpfprestsel']>($_SESSION['datos_bosque']['supselstpfprefpas']+$_SESSION['datos_bosque']['supselstumrefpas']+$_SESSION['datos_bosque']['supselstpfpref']+$_SESSION['datos_bosque']['supselstumref']))
				...
				/* elseif( ($_SESSION['datos_bosque']['Superficie']>50)  &&  ((round($_SESSION ['datos_bosque']['suptptotalreftpfp'],4)) < (round($_SESSION['datos_bosque']['suptpfp']*0.1,4)+round($_SESSION['datos_bosque']['suptpfppas']*0.1,4)))  )
					{
						$mostrar = round($_SESSION['datos_bosque']['suptpfp']*0.1,4)+round($_SESSION['datos_bosque']['suptpfppas']*0.1,4);
						$mensajeerror="VERIFIQUE: La superficie total para la restitucion de Bosques en TPFP es menor a la minima requerida (10%) equivalente a = ".$mostrar." Ha";
					} */
	...
	if ($mensajeerror!="")
		 ...
		 $_SESSION['superficie337']->Grabar($cn,337,$_SESSION['idreg'],$_SESSION['datos_bosque']['supprodali'],$_SESSION['datos_bosque']['suptotalrestfuera']) ;
		 if($_SESSION['Causal']==2){
			$_SESSION['superficie502']->Grabar($cn,502,$_SESSION['idreg'],$_SESSION['datos_bosque']['supprodali'],$_SESSION['datos_bosque']['suptotalrestfuera']) ;
		 }
			   ...
			   if(isset($_SESSION['datos_bosque']["chk239"])){
				...
				/* INSERTANDO ESCENARIOS O ALTERNATIVAS DE RESTITUCION DE RESTITUCION  */
								...
								if(isset($_SESSION['datos_bosque']["chk321"])){
									...
									if($_SESSION['Causal']==2){
										 $insertaux=sprintf("select * from f_guardar_escenarios (%s, %s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s );",
																			 GetSQLValueString(trim($_SESSION['idreg']), "int"),
																			 ...
																			 GetSQLValueString($_SESSION['datos_bosque']['suprest'],"double"),
																			 GetSQLValueString($_SESSION['datos_bosque']['supregnat'],"double"),
																			 GetSQLValueString($_SESSION['datos_bosque']['suprestpas'],"double"),
																			 GetSQLValueString($_SESSION['datos_bosque']['supregnatpas'],"double"),
																			 GetSQLValueString($_SESSION['datos_bosque']['suprestfuera'],"double"),
																			 GetSQLValueString($_SESSION['datos_bosque']['supregnatfuera'],"double")
																		 		);
									  }
										else
										{
