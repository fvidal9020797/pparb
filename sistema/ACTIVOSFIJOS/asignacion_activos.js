...
// 12 item 
  ...
  var colorr;
    ...
    optVer.setAttribute('src', '../images/logosdos/.png');   
 ...
 function cargarcomboListadoAsignacion(){
   ...
   // var nombregrupo=document.getElementById("nombreg").value;
    ...
    ClearSelect("buscararea");   
    
    // var idgrupo=document.getElementById("cbogrupo").value;   
  // alert(document.getElementById("idaccion1").innerHTML);
         var parametros = {
            "tarea" : "cargarcombosListadoAsignacion" 
        };
        $.ajax({
                data:  parametros,
                url:   'page_registro_activo_Ajax.php',
...
function lanzaraddactivo(){  
   // alert('asds');
    window.open("listado_activo_ext.php","Elegir Activos","width=1100,height=500,scrollbars=yes,toolbar=no,status=no");        
}
 ...
 function cargarListadoActivo_ext(paginaactual,pag_ini,pag_fin){
     limpiar("tableActivo") ;
 ...
 function cargarFilaActivo_ext2(indice,idIngreso, coducab, codmdryt, nombre_,idgrup , grupo, idsub, subgrupo,idobs ,nombreobs ,idestado,nombreesta ,sw1_) {
// 12 item 
    // alert(idIngreso + '-' + coducab + '-' + codmdryt + '-' + nombre + '-' + idgrup + '-' + grupo+ '-' + idsub+ '-' + subgrupo+ '-' + idarea+ '-' + area );
     tabla = document.getElementById('tableActivo');
     //alert('entraa'+sw1_);
    ...
    td0a.href="javascript:actualizaPadre("+idIngreso+",'"+coducab+"','"+codmdryt+"','"+nombre_+"')";
   ...
   // td2.innerHTML = coducab; 
    ...
    td2a.href="javascript:actualizaPadre("+idIngreso+",'"+coducab+"','"+codmdryt+"','"+nombre_+"')";
   ...
   // td3.innerHTML = codmdryt; 
    ...
    td3a.href="javascript:actualizaPadre("+idIngreso+",'"+coducab+"','"+codmdryt+"','"+nombre_+"')";
   ...
   // td4.innerHTML = nombre_; 
    ...
    td4a.href="javascript:actualizaPadre("+idIngreso+",'"+coducab+"','"+codmdryt+"','"+nombre_+"')";
   ...
   // td5.innerHTML = grupo; 
    ...
    td5a.href="javascript:actualizaPadre("+idIngreso+",'"+coducab+"','"+codmdryt+"','"+nombre_+"')";
   ...
   // td6.innerHTML = subgrupo; 
    ...
    td6a.href="javascript:actualizaPadre("+idIngreso+",'"+coducab+"','"+codmdryt+"','"+nombre_+"')";
   ...
   // td7.innerHTML = nombreobs; 
    ...
    td7a.href="javascript:actualizaPadre("+idIngreso+",'"+coducab+"','"+codmdryt+"','"+nombre_+"')";
   ...
   // td8.innerHTML = nombreesta; 
    ...
    td8a.href="javascript:actualizaPadre("+idIngreso+",'"+coducab+"','"+codmdryt+"','"+nombre_+"')";
...
function actualizaPadre(id_,coducab_, codm_, nombre_ ){
...
function deleteRow1(tableID) {  
 ...
 function guardarasignacion(){
    ...
    iddpto_=document.getElementById("cbodpto").value;    
    idregional_=document.getElementById("cboregional").value;  
    descripcion_=document.getElementById("message").value;    
    accion_=document.getElementById("idregistro").value;
    area_=document.getElementById("cboarea").value;
  ...
  var error_="";
    ...
    // alert(accion_);
       if(error_.length>0){         
          ...
          //-----GUARDAR ASIGNACION ---///
           var parametros = {
            "tarea" : "guardarAsignacionActivo" ,
            "txtfecha_" : txtfecha_ ,
            "idpersonal_" : idpersonal_ ,
            "iddpto_" : iddpto_ ,
            "idregional_" :idregional_ , 
            "ids" : ids,
            "descripcion" : descripcion_ ,
            "idarea" : area_ ,
            "acciongral" : accion_
                        ...
                        if(response1>0){   
                           ...
                           document.getElementById("idregistro").value=response1;
                            ...
                            $('#btnguardarimp').attr("disabled", true);
                             $("#btnguardarind").css({'background':'#A5A5A5'});
                             $("#btnguardarimp").css({'background':'#A5A5A5'});
                        // mensajeAlerta("Datos Guardado Correctamente2.!")
                         setTimeout ('nada ()', 2000); 
                          location.href = 'listado_asignacion.php';
  ...
  function guardarasignacionimprimir(){
    txtfecha_= document.getElementById("txtfecha").value;
    idpersonal_=document.getElementById("cboPersonal").value;
    iddpto_=document.getElementById("cbodpto").value;    
    idregional_=document.getElementById("cboregional").value;  
                    ...
                    success:  function (response1) {
                        ...
                        if(response1>0){     
                          ...
                          // document.getElementById('div_load').innerHTML = 'Cargando...';
                           ...
                           document.getElementById("idregistro").value=response1;
                           document.getElementById("idasig").value=response1;
                     ...
                     //   mensajeAlerta("Datos Guardado Correctamente2.!");
                        // setTimeout ('nada ()', 2000); 
                          //document.form4.submit();     
                           ...
                           // location.href = 'listado_asignacion.php';
                         //   setTimeout ('nada ()', 2000); 
                          location.href = 'listado_asignacion.php?ida='+response1;
...
function imprimirasignacion(idasig){
     //-----GUARDAR ASIGNACION ---///
    // alert(idasig);
           var parametros = {
            "tarea" : "imprimirAsignacion" ,
            "idasig" : idasig  
            };
            $.ajax({
                    data:  parametros,
                    ...
                    success:  function (response1) {
                        // eval(response1);
                        if(response1=="si"){                   
                             mensajeAlerta("Impresion Correctamente.!")
                             
                        }else{ 
                            mensajeAlerta("Error al Imprimir los Datos.!");
                        }
 ...
 function cargarListadoAsignacion(paginaactual,pag_ini,pag_fin){
   ...
   var filtro="";
     ...
     dptoa = document.getElementById('buscardpto').value;
     rega = document.getElementById('buscarregional').value; 
     area = document.getElementById('buscararea').value; 
     
        
        if(coda.length>0){            
             filtro=filtro+" and  upper(a.codigoasignacion) like  '%'|| upper('"+coda+"')||'%' ";  
        }
         if(fechaa.length>0){            
             filtro=filtro+" and  a.fechaasignacion='"+fechaa+"' ";
        }
        if(personala >0){            
             filtro=filtro+"  and  a.idfuncionario="+personala+" ";
        } 
        if(dptoa >0){            
             filtro=filtro+" and   a.iddpto = "+dptoa+" ";
        }
        
        if(area >0){            
             filtro=filtro+" and   a.idarea = "+area+" ";
        }
        
        
        if(rega >0){            
             filtro=filtro+" and   a.idregional = "+rega+" ";
        }
         
        
        var parametros = {
            "tarea" : "cargarListadoAsignacion" ,
            "filtro" : filtro ,
            "paginaactual" : paginaactual ,
            "paginaini" : pag_ini ,
            "paginafin" : pag_fin  
...
function cargarFilaAsignacion(indice,idIngreso, coducab, fecha_, nombrepersonal_,  depto_, regional_,obs,cant_, sw_) {
// 12 item 
    // alert(idIngreso + '-' + coducab + '-' + codmdryt + '-' + nombre + '-' + idgrup + '-' + grupo+ '-' + idsub+ '-' + subgrupo+ '-' + idarea+ '-' + area );
    ...
    optVer.onclick = function () { window.location.href='asignacion_activo.php?id=' + idIngreso; } //{CancelarFila()}    
    td1.appendChild(optVer);
    ...
    frm1.setAttribute('action', 'imprimir_acta_asignacion.php' );
  ...
  td4.style.background=colorr;
    ...
    td5 = document.createElement('td');
    td5.innerHTML = depto_; 
    td5.setAttribute('id', 'tdid5-' + idIngreso);
  td5.style.background=colorr;

    td6 = document.createElement('td');
    td6.innerHTML = regional_; 
    td6.setAttribute('id', 'tdid6-' + idIngreso);
    td6.style.background=colorr;
 ...
 function insertarAnteriorAS(name, pagina ,ini,fin) {
   ...
   // a.setAttribute('style', 'margin:6px;cursor:pointer;background-color:' + color);
    ...
    a.innerHTML = "<< Anterior";
    a.setAttribute('id', 'pagina' + pagina);
    a.onclick = function () { cargarListadoAsignacion(pagina,ini,fin); };
    tabla.appendChild(a);
}
...
function CargaEditarAsignacion(){
    var prodId = getParameterByName('id');
     if(prodId.length>0){
         document.getElementById("idregistro").value=prodId;
          document.getElementById("idtitulo").innerHTML="EDITAR";
...
function cargarCaracteristicas2(desc1){
    //alert(desc1);
...
// addRow("tableActivo",coducab_,codm_,nombre_,id_);
  function cargarfilaDetalleEditar(tableID,dato1,dato2,dato3,dato4,dato5) {
	...
	var row = table.insertRow(lastRow);
        ...
        var celln = row.insertCell(0);
        celln.style.background=colorf;
        
	var cell = row.insertCell(1);
         cell.style.background=colorf;
	var element = document.createElement("input");
	element.type = "checkbox";
      //  alert(dato5);
        if(dato5=="Devuelto"){
            element.disabled="disalbed";
        }
...
///------   paginado busqeda ext------
  ...
  function insertarAnterior_ext(name, pagina ,ini,fin) {
    tabla = document.getElementById(name);
    a = document.createElement('a');
   // a.setAttribute('style', 'margin:6px;cursor:pointer;background-color:' + color);
    a.setAttribute('class','next');
